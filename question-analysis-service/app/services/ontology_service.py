...
def normalize_and_merge(clusters: list[dict], questions: list[str]) -> list[dict]:
    ...
    for cluster in clusters:
        ...
        bucket = merged.setdefault(
            parent,
            {
                ...
                "sources": [],
                "domain": cluster.get("domain", "Auto-Discovered"),
                "source": cluster.get("source", "ml"),
        ...
        bucket["sources"].append(cluster.get("source", "ml"))
        bucket["domain"] = cluster.get("domain", bucket.get("domain", "Auto-Discovered"))
        if cluster.get("source"):
            bucket["source"] = cluster["source"]
        if cluster.get("confidence") is not None:
            bucket["confidence"] = cluster["confidence"]
    ...
    for bucket in merged.values():
        ...
        if "gemini" in sources:
            bucket["source"] = "gemini"
        elif "ml_fallback" in sources:
            bucket["source"] = "ml_fallback"
        elif sources:
            bucket["source"] = sources[0]
        else:
            bucket["source"] = "ml"
